...
The left panel shows the selection-frequency heatmap $\hat{\pi}_{ij}$. The subsequent panels threshold these frequencies at increasing stability cutoffs ($\hat{\pi}_{ij} \ge 0.5, 0.75, 0.9, 0.95, 0.995$). Edges that consistently survive at high thresholds correspond to stable conditional-dependence relationships. In this dataset, strong sector-level structures remain highly stable—particularly the mega-cap technology group, the JPM–BAC banking link, the XOM–CVX energy pair, and the consumer-staples/healthcare connections—while weaker cross-sector edges disappear as the threshold increases. This confirms that the major clusters identified by the PC algorithm reflect reproducible features of the data.
